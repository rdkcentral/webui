		...
		if(($i == 1 || $i == 2) || ($i > 2 && $_SESSION["loginuser"] == "mso")){
			...
			if(validFilterParam($ft_config_filtered['ft'])){
				foreach ($ssids as $i)	//incase some filter rule apply to more than one SSID (such as HotSpot)
				{
					...
					{
						$new_inst = addTblObj("Device.WiFi.AccessPoint.$i.X_CISCO_COM_MacFilterTable.");
						if ($new_inst > 0)
						{
							setStr("Device.WiFi.AccessPoint.$i.X_CISCO_COM_MacFilterTable.$new_inst.DeviceName", $ft[$j][0], false);
							setStr("Device.WiFi.AccessPoint.$i.X_CISCO_COM_MacFilterTable.$new_inst.MACAddress", $ft[$j][1], true);
						}
					}
